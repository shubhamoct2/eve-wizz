...
namespace Modules\Abstracts\Traits {
    use Modules\Abstracts\Repositories\Repository;
    use Modules\Abstracts\Exceptions\CoreInternalErrorException;
    ...
    trait HasRequestCriteriaTrait
    {
        ...
        #[
            Deprecated(
                reason: 'Use addRequestCriteria() on the Repository',
                replacement: "%class%->repository->addRequestCriteria();"
            )
        ]
        public function addRequestCriteria(
            $repository = null,
            array $fieldsToDecode = ["id"]
        ): static {
            ...
            if (is_null($repository)) {
                if (!isset($this->repository)) {
                    throw new CoreInternalErrorException(
                        "No protected or public accessible repository available"
                    );
                }
        ...
        {
            return $this->hashIdEnabled() &&
                $this->isSearching(request()->query());
        }
                ...
                if (array_key_exists($field, $searchArray)) {
                    if (empty(Hashids::decode($searchArray[$field]))) {
                        throw new \InvalidArgumentException(
                            "Only hash ids are allowed. {$field}:$searchArray[$field]"
                        );
                    }
                    $searchArray[$field] = Hashids::decode(
                        $searchArray[$field]
                    )[0];
                }
            }
